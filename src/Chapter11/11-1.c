// Implementation of Prim-Jarnik Algorithm
...
// Implement Prim-Jarnik Algorithm
int prim_jarnik(vertex* vert_arr, int vert_arr_size, vertex* begin)
{
    int total_weight = 0;
    heap* h = heap_init();
    ...
    while (!heap_is_empty(h))
    {
        ...
        while (inc)
        {
            ...
            if (opposite->heap_pos && inc->edge->weight < opposite->distance)
            {
                heap_update_position(h, opposite->heap_pos, inc->edge->weight);
